...
#define BSP_LED_CHASE_SPEED_MS      100
...
void bsp_led_init()
{
  ...
  ESP_ERROR_CHECK(rmt_new_led_encoder(&encoder_config, &led_encoder));
...
{
	uint8_t led_pixels[BSP_LED_COUNT * 3];
	uint32_t red = 0;
	uint32_t green = 0;
	uint32_t blue = 0;
	uint16_t hue = 0;
